...
#tprnacntfile="/data/tempa0d9/TCGA-A7-A0D9_TPstar_tophat_merged_grp_dedup_karyotic_bq20_dp5_Arefbase.vcf"
#tpdnacntfile="/data/tempa0d9/TCGA-A7-A0D9_TP_bq25dp4_dnaaref_readcnt.vcf"
#tpoutfile="/data/tempa0d9/TCGA-A7-A0D9_TP_bq25dp4_dnaaref_allaz.vcf"
...
cat(sprintf("Reading DNA cnt file: %s\n", tpdnacntfile))
exnt_alldp=fread(file=tpdnacntfile,header=FALSE, sep="\t", skip=1, select=c(1,2,3,6))
cat(sprintf("Reading DNA cnt file done\n"))
exnt_alldp=as.data.frame(exnt_alldp)
#exnt_alldp=exnt_alldp[,-7]
colnames(exnt_alldp)=c("chr","pos",   "coverage", "gcnt")
...
cat(sprintf("reading RNA count file: %s\n",tprnacntfile))
stnt_alldp=fread(file=tprnacntfile, select=c("chrom","position","q20_depth","Altbase","Refcnt","Altcnt","Alt_SB(pos/(pos+neg))","Alt_ER(alt/q20depth)","ref_plus","ref_minus","alt_plus","alt_minus"),header=TRUE,sep="\t")
#stnt_alldp$position=paste(stnt_alldp$chrom,stnt_alldp$position,sep=":")
stnt_alldp=as.data.frame(stnt_alldp)
#stnt_alldp=stnt_alldp[stnt_alldp$Altcnt>0,]
#stnt_alldp=subset(stnt_alldp, select=-chrom)
...
write.table(exnt_alldp,file=tpoutfile,row.names = FALSE,col.names = TRUE,sep="\t",quote=FALSE)
